...
final class NeuralRenderingTransformerOperatorTests: XCTestCase {
  ...
  func testExternalVendorCosineNormalizeMatchesCUDAFragmentCandidate() throws {
    guard
      let path = ProcessInfo.processInfo.environment["NRK_BLOCK48_NORM_FIXTURE"]
    else {
      throw XCTSkip(
        "set NRK_BLOCK48_NORM_FIXTURE to run the private normalization probe"
      )
    }
  ...
  func testExternalRecoveredPostBlockExecutesWhenConfigured() throws {
    guard let path = ProcessInfo.processInfo.environment["NRK_LOGICAL_WEIGHTS"] else {
      throw XCTSkip("set NRK_LOGICAL_WEIGHTS to run the local recovered-weight probe")
    }
  ...
  func testExternalFacePreStageDistributionWhenConfigured() throws {
    ...
    guard let weightsPath = environment["NRK_LOGICAL_WEIGHTS"],
      let colorPath = environment["NRK_FACE_COLOR"],
      let captureRoot = environment["NRK_FACE_PRE_CAPTURE_ROOT"]
    else {
      throw XCTSkip(
        "set NRK_LOGICAL_WEIGHTS, NRK_FACE_COLOR, and NRK_FACE_PRE_CAPTURE_ROOT"
      )
    }
  ...
  func testExternalWindowStagesAgainstVendorInputsWhenConfigured() throws {
    let environment = ProcessInfo.processInfo.environment
    guard let weightsPath = environment["NRK_LOGICAL_WEIGHTS"],
      let stageRoot = environment["NRK_WINDOW_STAGE_ROOT"]
    else {
      throw XCTSkip("set NRK_LOGICAL_WEIGHTS and NRK_WINDOW_STAGE_ROOT")
    }
    let arrays = try loadArrays(
      url: URL(fileURLWithPath: weightsPath),
      stream: .cpu
  ...
  func testExternalRecoveredPackageRunsThroughPublicBackend() async throws {
    guard let path = ProcessInfo.processInfo.environment["NRK_NEURAL_RENDERING_PACKAGE"] else {
      throw XCTSkip("set NRK_NEURAL_RENDERING_PACKAGE to run the package integration probe")
    }
  ...
  func testExternalRecoveredStageTimingProfile() throws {
    let environment = ProcessInfo.processInfo.environment
    guard environment["NRK_PROFILE_NEURAL_RENDERING_STAGES"] == "1",
      let path = environment["NRK_LOGICAL_WEIGHTS"]
    else {
      throw XCTSkip("set external weights and NRK_PROFILE_NEURAL_RENDERING_STAGES=1")
    }
    let arrays = try loadArrays(url: URL(fileURLWithPath: path), stream: .cpu)
    let weights = ValidatedWeights(arrays: arrays)
    let fuseWindowFFN = environment["NRK_PROFILE_NEURAL_RENDERING_FUSED"] == "1"
    let encoder = try NeuralRenderingEncoder(
      weights: weights,
    ...
    for iteration in 0..<5 {
      ...
      print("nrk-stage-profile iteration=\(iteration) " + values.joined(separator: " "))
    }
  }
}
