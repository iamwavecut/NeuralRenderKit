...
/// `NRK_LOGICAL_WEIGHTS` and `NRK_BLOCK_TIMING=HEIGHTxWIDTH`. Diagnostics only.
final class NeuralRenderingBlockTimingTests: XCTestCase {
  func testExternalBlockTimingWhenConfigured() throws {
    let environment = ProcessInfo.processInfo.environment
    guard let weightsPath = environment["NRK_LOGICAL_WEIGHTS"], let shape = environment["NRK_BLOCK_TIMING"] else {
      throw XCTSkip("set NRK_LOGICAL_WEIGHTS and NRK_BLOCK_TIMING=HEIGHTxWIDTH")
    }
