...
/// weights (`NRK_LOGICAL_WEIGHTS`, optional `NRK_FUSED_TIMING=HEIGHTxWIDTH`).
final class NeuralRenderingFusedWindowBlockTests: XCTestCase {
  ...
  func testExternalWeightsParityAndTimingWhenConfigured() throws {
    ...
    guard let weightsPath = environment["NRK_LOGICAL_WEIGHTS"] else {
      throw XCTSkip("set NRK_LOGICAL_WEIGHTS (and NRK_FUSED_TIMING=HEIGHTxWIDTH)")
    }
    let shape = environment["NRK_FUSED_TIMING"] ?? "544x960"
