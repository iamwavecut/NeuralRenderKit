...
///
/// Set `NRK_LOGICAL_WEIGHTS` and `NRK_WINDOW_TIMING=HEIGHTxWIDTH` (the
...
final class NeuralRenderingWindowBlockTimingTests: XCTestCase {
  func testExternalWindowBlockCostSplitWhenConfigured() throws {
    ...
    guard let weightsPath = environment["NRK_LOGICAL_WEIGHTS"],
      let shape = environment["NRK_WINDOW_TIMING"]
    else {
      throw XCTSkip("set NRK_LOGICAL_WEIGHTS and NRK_WINDOW_TIMING=HEIGHTxWIDTH")
    }
    ...
    guard parts.count == 2, parts[0] > 0, parts[1] > 0,
      ...
      throw XCTSkip("NRK_WINDOW_TIMING must look like 544x960 with multiples of 8")
    }
    let height = parts[0]
    let width = parts[1]
