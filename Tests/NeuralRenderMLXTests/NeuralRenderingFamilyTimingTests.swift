...
/// `NRK_LOGICAL_WEIGHTS` and `NRK_FAMILY_TIMING=HEIGHTxWIDTH`.
final class NeuralRenderingFamilyTimingTests: XCTestCase {
  func testEncoderFamiliesWhenConfigured() throws {
    let environment = ProcessInfo.processInfo.environment
    guard let weightsPath = environment["NRK_LOGICAL_WEIGHTS"], let shape = environment["NRK_FAMILY_TIMING"] else {
      throw XCTSkip("set NRK_LOGICAL_WEIGHTS and NRK_FAMILY_TIMING=HEIGHTxWIDTH")
    }
