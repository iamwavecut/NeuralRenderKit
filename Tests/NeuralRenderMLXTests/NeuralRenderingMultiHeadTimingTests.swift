...
/// `NRK_LOGICAL_WEIGHTS` and `NRK_MULTIHEAD_TIMING=BLOCK:CHANNELS:HIDDEN:HEADS:HEIGHTxWIDTH` (e.g. `15:256:704:8:68x120`).
final class NeuralRenderingMultiHeadTimingTests: XCTestCase {
  func testMultiHeadBlockCostSplitWhenConfigured() throws {
    ...
    guard let weightsPath = environment["NRK_LOGICAL_WEIGHTS"], let spec = environment["NRK_MULTIHEAD_TIMING"] else {
      throw XCTSkip("set NRK_LOGICAL_WEIGHTS and NRK_MULTIHEAD_TIMING=BLOCK:CHANNELS:HIDDEN:HEADS:HxW")
    }
