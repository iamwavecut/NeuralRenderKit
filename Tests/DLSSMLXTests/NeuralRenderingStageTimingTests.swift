...
///
/// Set `MLXDLSS_LOGICAL_WEIGHTS` to the logical safetensors and `MLXDLSS_STAGE_TIMING`
...
final class NeuralRenderingStageTimingTests: XCTestCase {
  func testExternalStageTimingWhenConfigured() throws {
    ...
    guard let weightsPath = environment["MLXDLSS_LOGICAL_WEIGHTS"],
      let shape = environment["MLXDLSS_STAGE_TIMING"]
    else {
      throw XCTSkip("set MLXDLSS_LOGICAL_WEIGHTS and MLXDLSS_STAGE_TIMING=HEIGHTxWIDTH")
    }
    let parts = shape.split(separator: "x").compactMap { Int($0) }
    guard parts.count == 2, parts[0] > 0, parts[1] > 0 else {
      throw XCTSkip("MLXDLSS_STAGE_TIMING must look like 1088x1920")
    }
    let height = parts[0]
    let width = parts[1]
    ...
    let checksums = environment["MLXDLSS_STAGE_CHECKSUMS"] != nil
    ...
    let iterations = Int(environment["MLXDLSS_STAGE_ITERATIONS"] ?? "2") ?? 2
    for iteration in 0..<iterations {
      if environment["MLXDLSS_STAGE_CLEAR"] != nil { GPU.clearCache() }
