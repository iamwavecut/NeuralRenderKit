...
final class CLITests: XCTestCase {
  func testRunSequenceRejectsUnknownNetworkGeometry() throws {
    let result = try runCLI([
      "run-sequence", "/definitely/missing/model.nrkmodel",
  ...
  func testStreamRequiresModelWidthAndHeightAndKnownOptions() throws {
    ...
    let extent = try runCLI(["stream", "/definitely/missing/model.nrkmodel", "--width", "0", "--height", "16"])
    XCTAssertEqual(extent.status, 2)
    XCTAssertTrue(extent.stderr.contains("positive --width and --height"), extent.stderr)
    let unknown = try runCLI(["stream", "/definitely/missing/model.nrkmodel", "--width", "16", "--height", "16", "--layout", "x"])
    XCTAssertEqual(unknown.status, 2)
    XCTAssertTrue(unknown.stderr.contains("unknown stream option"), unknown.stderr)
    let scaled = try runCLI(["stream", "/definitely/missing/model.nrkmodel", "--width", "16", "--height", "16", "--processing-scale", "2"])
  ...
  func testRenderImageRequiresOutputOption() throws {
    let result = try runCLI([
      "render-image", "/definitely/missing/image.png", "/definitely/missing/model.nrkmodel",
    ])
  ...
  func testExternalNeuralRenderingPackageRendersStillImage() throws {
    guard
      ...
      throw XCTSkip("set NRK_NEURAL_RENDERING_PACKAGE to run the image CLI probe")
    }
  ...
  func testRGBFirstFrameInputRequiresExplicitTensorFiles() throws {
    let result = try runCLI([
      "run", "/definitely/missing/model.nrkmodel",
  ...
  func testExternalNeuralRenderingPackageAcceptsRecoveredRGBFirstFrameInput() throws {
    guard
      ...
      throw XCTSkip("set NRK_NEURAL_RENDERING_PACKAGE to run the package CLI probe")
    }
    let temporaryDirectory = try makeTemporaryDirectory()
    defer { try? FileManager.default.removeItem(at: temporaryDirectory) }
  ...
  func testExternalCoreMLNeuralRenderingPackageRunsRawHead() throws {
    guard
      let packagePath = ProcessInfo.processInfo.environment[
        "NRK_NEURAL_RENDERING_COREML_PACKAGE"
      ]
    else {
      throw XCTSkip("set NRK_NEURAL_RENDERING_COREML_PACKAGE to run the CLI probe")
    }
    let temporaryDirectory = try makeTemporaryDirectory()
    defer { try? FileManager.default.removeItem(at: temporaryDirectory) }
    let inputURL = temporaryDirectory.appendingPathComponent("input.f32")
    ...
    throws
  {
    guard
      ...
      throw XCTSkip("set NRK_NEURAL_RENDERING_PACKAGE to run the jitter CLI probe")
    }
    let temporaryDirectory = try makeTemporaryDirectory()
    defer { try? FileManager.default.removeItem(at: temporaryDirectory) }
    let width = 128
  ...
  func testMalformedPackageExitsTwoWithoutCrashDump() throws {
    let temporaryDirectory = try makeTemporaryDirectory()
    defer { try? FileManager.default.removeItem(at: temporaryDirectory) }
    let packageURL = temporaryDirectory.appendingPathComponent("Malformed.nrkmodel")
  ...
  func testRunReadsAndWritesExactRawFloat32Tensor() async throws {
    let temporaryDirectory = try makeTemporaryDirectory()
    defer { try? FileManager.default.removeItem(at: temporaryDirectory) }
    let packageURL = temporaryDirectory.appendingPathComponent("Demo.nrkmodel")
  ...
  private func cliExecutableURL() -> URL {
    ...
    return url.appendingPathComponent(".build/debug/nrk")
  }
