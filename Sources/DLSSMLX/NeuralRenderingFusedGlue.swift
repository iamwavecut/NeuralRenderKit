...
enum NeuralRenderingFusedGlue {
  /// `MLXDLSS_FUSED_GLUE=0` restores the per-operation chains (diagnostics).
  nonisolated(unsafe) static var enabled: Bool =
    ProcessInfo.processInfo.environment["MLXDLSS_FUSED_GLUE"] != "0"
  ...
  private static let poolKernel = MLXFast.metalKernel(
    ...
    source: #"""
      ...
      if (emitFull) {
        full4[base00] = half4(half(float(mlxdlss_e4m3(v00.x))), half(float(mlxdlss_e4m3(v00.y))), half(float(mlxdlss_e4m3(v00.z))), half(float(mlxdlss_e4m3(v00.w))));
        full4[base01] = half4(half(float(mlxdlss_e4m3(v01.x))), half(float(mlxdlss_e4m3(v01.y))), half(float(mlxdlss_e4m3(v01.z))), half(float(mlxdlss_e4m3(v01.w))));
        full4[base10] = half4(half(float(mlxdlss_e4m3(v10.x))), half(float(mlxdlss_e4m3(v10.y))), half(float(mlxdlss_e4m3(v10.z))), half(float(mlxdlss_e4m3(v10.w))));
        full4[base11] = half4(half(float(mlxdlss_e4m3(v11.x))), half(float(mlxdlss_e4m3(v11.y))), half(float(mlxdlss_e4m3(v11.z))), half(float(mlxdlss_e4m3(v11.w))));
      }
      ...
      pooled4[index] = half4(half(float(mlxdlss_e4m3(mean.x))), half(float(mlxdlss_e4m3(mean.y))), half(float(mlxdlss_e4m3(mean.z))), half(float(mlxdlss_e4m3(mean.w))));
      """#,
  ...
  private static let upsampleMergeKernel = MLXFast.metalKernel(
    ...
    source: #"""
      ...
      if (publish) {
        value = half4(half(float(mlxdlss_e4m3(value.x))), half(float(mlxdlss_e4m3(value.y))), half(float(mlxdlss_e4m3(value.z))), half(float(mlxdlss_e4m3(value.w))));
      }
      output4[index] = value;
      """#,
    header: NeuralRenderingTransformerOperations.e4m3MetalHeaderText
