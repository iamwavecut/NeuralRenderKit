...
/// `mlxdlss framegen A.png B.png --weights framegen.safetensors --output OUT.png [--phase 0.5] [--factor N] [--precision float16|float32]`
///
...
enum FrameGenCommand {
  ...
  static func run(arguments: [String]) async throws {
    ...
    guard let weights else { throw CLIError.usage("framegen requires --weights framegen.safetensors (mlxdlss-weights extract-fg)") }
    ...
    var outputs: [String] = []
    var seconds = 0.0
    if repeatCount > 1, ProcessInfo.processInfo.environment["MLXDLSS_FG_BENCH_STAGES"] == "1" {
