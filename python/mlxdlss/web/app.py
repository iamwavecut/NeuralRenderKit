"""``mlxdlss-web``: start the local front end.

    mlxdlss-web [--port 8181] [--host 127.0.0.1] [--native] [--no-browser] [--root DIR]
...
through ``MLXDLSS_NR_WEIGHTS``, ``MLXDLSS_NR_MODEL`` and ``MLXDLSS_FG_WEIGHTS``; jobs and
results live under ``~/MLX-DLSS`` (``--root`` / ``MLXDLSS_WEB_ROOT``).
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="mlxdlss-web", description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    parser.add_argument("--root", type=Path, default=None, help="folder for settings, jobs and results (default ~/MLX-DLSS)")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    register_pages()
    ui.run(
        host=args.host, port=args.port, title="MLX-DLSS", dark=settings.theme_dark, reload=False,
